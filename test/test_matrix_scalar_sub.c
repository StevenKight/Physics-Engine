...
static char *test_matrix_scalar_sub_cpu() {
    int n = 2, m = 2;
    ...
    matrix_scalar_sub(A, &scalar, C, n, m, false);

    printf("C after CPU scalar sub: %f %f %f %f\n", C[0], C[1], C[2], C[3]);
...
static char *test_matrix_scalar_sub_gpu() {
    ...
    matrix_scalar_sub(&A_matrix, &scalar, &C_matrix, n, m, true); // use_gpu = true to call CUDA wrapper
...
static char *all_tests() {
    mu_run_test(test_matrix_scalar_sub_cpu);
    mu_run_test(test_matrix_scalar_sub_gpu);
    return 0;
}
