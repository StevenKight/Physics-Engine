...
static char *test_matrix_add_cpu() {
    int n = 2, m = 2;
    ...
    printf("C after CPU addition: %f %f %f %f\n", C[0], C[1], C[2], C[3]);
...
static char *test_matrix_add_gpu() {
    ...
    matrix_add(&A_matrix, &B_matrix, &C_matrix, n, m, true); // use_gpu = true to call CUDA wrapper
...
static char *all_tests() {
    mu_run_test(test_matrix_add_cpu);
    mu_run_test(test_matrix_add_gpu);
    return 0;
}
